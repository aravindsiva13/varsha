...
class _CreditsListScreenState extends State<CreditsListScreen> {
  ...
  Future<void> _fetchCreditsList() async {
    ...
    try {
      final url = Uri.parse(
          "${ApiConfig.getBaseUrl()}/CreditsAdding/list?page=$_currentPage&limit=$_limit");
      final response = await http.post(
        url,
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({}),
  ...
  void _openDetails(Map<String, dynamic> credit) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CreditsDetailsScreen(credit: credit),
      ),
    ).then((_) => _fetchCreditsList());
  }
  ...
  void _openForm() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CreditsDetailsScreen(credit: {}),
      ),
    ).then((_) => _fetchCreditsList());
  }
...
class CreditsDetailsScreen extends StatefulWidget {
  final Map<String, dynamic> credit;
  const CreditsDetailsScreen({super.key, required this.credit});

  @override
  State<CreditsDetailsScreen> createState() => _CreditsDetailsScreenState();
}

class _CreditsDetailsScreenState extends State<CreditsDetailsScreen> {
  final _formKey = GlobalKey<FormState>();
  late TextEditingController _clientNameController;
  late TextEditingController _clientIdController;
  late TextEditingController _creditsController;
  late TextEditingController _creditsTypeController;
  ...
  bool get isExistingCredit =>
      widget.credit.isNotEmpty && widget.credit["id"] != null;

  @override
  void initState() {
    super.initState();
    _portal = widget.credit["portal"]?.toString() ?? "iCloud";
    _resetInterval = widget.credit["creditsResetInterval"]?.toString();
    _addInitialBalance = widget.credit["addInitialBalance"]?.toString();

    _clientNameController = TextEditingController(
        text: widget.credit["clientName"]?.toString() ?? "");
    _clientIdController = TextEditingController(
        text: widget.credit["clientId"]?.toString() ?? "");
    _creditsController = TextEditingController(
        text: widget.credit["credits"]?.toString() ?? "");
    _creditsTypeController = TextEditingController(
        text: widget.credit["creditsType"]?.toString() ?? "");
    _initialBalanceController = TextEditingController(
        text: widget.credit["BalanceNow"]?.toString() ?? "");
  }
  ...
  void dispose() {
    ...
    _creditsTypeController.dispose();
  ...
  Future<void> _downloadFile() async {
    final id = widget.credit["id"] as int?;
    final filePath = _isEditMode
        ? _pickedFileName ?? widget.credit["uploadFile"]
        : widget.credit["uploadFile"];
    ...
    try {
      final dio = Dio();
      final url = "${ApiConfig.getBaseUrl()}/CreditsAdding/download/$id";
      final response = await dio.get(url,
          options: Options(responseType: ResponseType.bytes));
      if (response.statusCode == 200) {
        final bytes = response.data as List<int>;
    ...
    try {
      ...
      final formData = FormData.fromMap({
        ...
        'initialBalance': _addInitialBalance == "Yes"
            ? int.tryParse(_initialBalanceController.text.trim()) ?? 0
            : 0,
        if (isExistingCredit) 'id': widget.credit["id"]?.toString(),
      });
      final url = isExistingCredit
          ? "${ApiConfig.getBaseUrl()}/CreditsAdding/update"
          : "${ApiConfig.getBaseUrl()}/CreditsAdding/create";
      final response = await dio.post(url, data: formData);
      if (response.statusCode == 200 || response.statusCode == 201) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(isExistingCredit
  ...
  Future<void> _deleteCredit() async {
    final confirmed = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
              ...
              content:
                  const Text("Are you sure you want to delete this credit?"),
              actions: [
                TextButton(
    ...
    try {
      final dio = Dio();
      final url =
          "${ApiConfig.getBaseUrl()}/CreditsAdding/delete/${widget.credit["id"]}";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(isExistingCredit ? "Credits Details" : "New Credits"),
        backgroundColor: const Color(0xFF009688),
        actions: [
          if ((isExistingCredit && !_isEditMode) || (_pickedFileName != null))
            IconButton(
        ...
        child: _isEditMode
                  ...
                  children: [
                    ...
                    if (_addInitialBalance == "Yes")
                      TextFormField(
                        controller: _initialBalanceController,
                        keyboardType: TextInputType.number,
                        decoration: _inputDecoration("Initial Balance *"),
                    ...
                    TextFormField(
                        controller: _creditsTypeController,
                        decoration: _inputDecoration("Credits Type *"),
                        validator: (v) =>
                    ...
                    ElevatedButton.icon(
                        ...
                        label: Text(isExistingCredit ? "Update" : "Save"),
                        onPressed: _submit),
                  ],
                ),
              )
            : ListView(
                children: [
                  ...
                  _readonlyCard("Credit Reset Interval", _resetInterval ?? ""),
                  _readonlyCard(
                      "Add Initial Balance", _addInitialBalance ?? ""),
                  if (_addInitialBalance == "Yes")
                    _readonlyCard(
                        "Initial Balance", _initialBalanceController.text),
                  _readonlyCard("Credits Type", _creditsTypeController.text),
                  _readonlyCard("Credits", _creditsController.text),
                  Card(
                    margin: const EdgeInsets.symmetric(vertical: 6),
                    shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(12)),
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(widget.credit["uploadFile"]?.toString() ??
                          "No file uploaded"),
                      trailing: IconButton(
                          icon: const Icon(Icons.download),
