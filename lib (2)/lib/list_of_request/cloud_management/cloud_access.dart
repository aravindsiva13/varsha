...
class _CloudAccessScreenState extends State<CloudAccessScreen> {
  ...
  Future<void> _fetchCloudList() async {
    ...
    try {
      final url = Uri.parse(
          "http://localhost:3008/cloudAccess/list?page=$_currentPage&limit=$_limit");
...
class CloudAccessDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredItems = Map<String, dynamic>.from(item)
      ..remove('created_at')
      ..remove('updated_at')
      ..remove('createdAt')
      ..remove('updatedAt');
...
class _CloudAccessFormScreenState extends State<CloudAccessFormScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      final response = await http.post(
        Uri.parse("http://localhost:3008/cloudAccess/create"),
        headers: {"Content-Type": "application/json"},
