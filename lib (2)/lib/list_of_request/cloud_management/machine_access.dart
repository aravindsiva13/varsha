...
class _MachineAccessScreenState extends State<MachineAccessScreen> {
  ...
  Future<void> _fetchMachineList() async {
    ...
    try {
      final url = Uri.parse(
          "http://localhost:3008/machineAccess/list?page=$_currentPage&limit=$_limit");
...
//         child: ListView(
//           children: machine.entries.map((entry) {
//             return Card(
...
class _MachineFormScreenState extends State<MachineFormScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      final response = await http.post(
        Uri.parse("http://localhost:3008/machineAccess/create"),
        headers: {"Content-Type": "application/json"},
