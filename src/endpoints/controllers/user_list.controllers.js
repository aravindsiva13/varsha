...
exports.getList = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Found ${count} total records, returning ${rows.length} records`
    );
...
exports.update = async (req, res) => {
  ...
  const { id, portal, clientName, clientId, replace } = req.body;

  try {
    const record = await req.db.UserList.findByPk(id);
    if (!record) {
  ...
  try {
    ...
    res.download(filePath, (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Error while sending the file." });
      }
